...
8. **公開（GitHubへ反映）**：対象は `katut-brain/katut-brain.github.io` リポ（Vaultリポではない）。
   ...
   **(c) reviews 無し・fetch_facts も無し（または `git status --porcelain -- fetch_facts/<TARGET>.json` が空＝今回のランで変更が無い）**：
   ...
   - 共通: 送信が一時失敗しても、生 `git push` にも `push_files` にも**戻らない**。`push_via_api.sh` を2回まで、それでもダメなら諦めて翌ランに回す（`unknown` のときは1回も再実行しない）。
8.5. **公開（Vaultリポへ・ブックマークノート）**（手順7.5でノートを新規作成した場合のみ実行）：対象は `katut-brain/obsidian-vault` リポ（手順8の `katut-brain.github.io` とは別リポ）。
   ...
   - 🚫 **ここでも `push_files` は使わない。** 失敗したときの扱いは**手順8の①〜③をそのまま適用する**（2回まで再実行／2回とも駄目なら諦める／`unknown` なら絶対に再実行しない）。
   - **押さなくてもそのノートは失われない** —— 手順7.5の重複チェックはリポ上の既存ノートを見るので、押していないノートは翌晩「まだ無い」と判定されて改めて対象になる。**押さない側にコストが無いのだから、確認できないまま押す理由も無い。**
