...
8.5. **公開（Vaultリポへ・ブックマークノート）**（push は手順7.5でノートを新規作成した場合のみ。ただし末尾の**到達性ログの1行だけは、押すノートが0件の夜でも必ず実行する**）：対象は `katut-brain/obsidian-vault` リポ（手順8の `katut-brain.github.io` とは別リポ）。
   - 押すファイルは、手順7.5でスキーマ検証に**合格**し新規作成した `Explore/bookmarks/rd-*.md` のみ（検証落ちのファイル・既存ファイルは含めない）。
   - **まず `push_via_api.sh` で押す**（2026-09-09 更新。以前の「今夜は経路を変えない」は撤回）：
     ```bash
     bash push_via_api.sh katut-brain/obsidian-vault "bookmark notes: <TARGET> (N件)" <押すパス...>
     ```
     終了コードで**必ず**分岐する。ここを間違えると二重コミットになる：
     - **0** → 完了。スクリプトが載せる前に SHA-256 で照合済みなので、追加の照合は要らない。`push_files` は**使わない**
     - **1** → `main` は1バイトも動いていない。**同じコマンドをもう1回だけ**実行する。それでも 1 なら**下のフォールバックへ落ちる**
     - **2** → 引数の誤り。`main` は動いていないので、同じくフォールバックへ落ちてよい
     - **3** → **判定不能。再実行もフォールバックも絶対にしない。** 載ったかどうか分からない状態で `push_files` を重ねると、
       二重コミットか他人の変更の踏み潰しになる。`WRITE_PATH: <パス> via=api verify=UNCHECKED note=unknown_do_not_retry` を残して、
       そのノートは翌晩に回す（手順7.5の重複チェックがあるので、既存ノートが壊されることはない）
   - **フォールバック（exit 1 が2回、または exit 2 のときだけ）**: 従来どおり `push_files` で押し、押した後に照合する：
     ```bash
     bash push_via_api.sh --verify-only katut-brain/obsidian-vault <押したパス>
     ```
     `verify=match` なら完了。`MISMATCH` なら押し直して再照合し、2回目も駄目なら諦めて `WRITE_PATH: <パス> fallback=push_files verify=GIVEUP` を残す。
     `unreadable` なら照合できないので `verify=UNCHECKED` と残す（成功と書かない）。
   - **なぜフォールバックを残すのか**（公開リポ側では廃止したのに、ここだけ残す理由）：
     Vaultリポは `gh api` が実運用で通るかまだ未確認で、切ってしまうと**構造的に毎晩失敗して外部脳への取り込みが永久に止まる**。
     一方フォールバック先は**今日までの経路そのもの**なので、落ちても悪化しない。API が通った夜だけ転記を経由しなくなる、という片側だけの改善になっている。
     （公開リポ側は主経路が「照合してから載せる」に変わった以上、フォールバックが唯一残る破損経路になるので廃止した。こことは事情が違う。）
   - **到達性のログを1行残す**（読み取りのみ・何も書かない）。押すノートが0件の夜でも実行する：
     ```bash
     bash push_via_api.sh --verify-only katut-brain/obsidian-vault RULES.md
     ```
     `RULES.md` はクローンにあり今回のランで触っていないので、一致するはず。`verify=match` なら API が届いている、
     `verify=unreadable` なら届いていない（公式仕様の「セッションに紐付いていないリポには 403」に該当する可能性が高い）。
     **どちらでも手順は止めない。**
   - コミットメッセージは `bookmark notes: <TARGET> (N件)` 形式。
   - ⚠️ **Vaultリポ側には Actions の検証ゲートが1本も無い**（`total_count: 0`・2026-09-08 実測）。公開リポと違って、壊れたノートを押しても誰も止めない。しかもローカルVaultへは Obsidian Git プラグインが10分以内に取り込む。**照合を省くとそのまま外部脳に入る**ので、照合は必ず行うこと（API 経路の exit 0 は載せる前に照合済みなので、これを満たしている）。
   - push 対象ノートが0件（新規0件・全件重複スキップ・全件検証落ちのいずれか）の場合は、このpushを行わない。
   - 送信が一時失敗しても生 `git push` には戻らない（403ループ防止）。試行回数は上の終了コード分岐が定める（API 2回 → フォールバック1〜2回）。それでも駄目なら諦めて翌ランに回す（取りこぼしたブックマークのノートは翌晩以降の手順7.5で改めて対象になる＝重複チェックにより既存ノートは壊されない）。
