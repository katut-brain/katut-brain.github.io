...
4. **URL分類**：手順3で抽出した各レコードの `source` URL を、取得前に次の2グループへ分類する。
   ...
   - **X動画**：`GEMINI_API_KEY`が設定されていれば、`text`に「動画の内容: ...」として映像+音声の理解結果が自動で埋め込まれる（Pythonスクリプト側で完結、追加のエージェント側操作は不要）。カード生成時はこの内容を読んだ上で一言にする。理解できなかった場合（`missing`に`"video_content"`が入る）はタイトル・ツイート本文のみで一言を作る。
   ...
   - **Instagram Reels動画**：`GEMINI_API_KEY`が設定されていれば、`text`に「動画の内容: ...」として映像+音声の理解結果が自動で埋め込まれる（Pythonスクリプト側で完結）。取得元は非公式サードパーティサービス(vxinstagram.com)経由のため、Xの動画取得より壊れやすい点に留意（`missing`に`"video_content"`が入ったら理解できなかった意味＝キャプションのみで一言を作る）。
...
4.2. **取得の生の事実を記録する（必須・小さいファイル1本）**：手順4で `fetch_content.py` が返した各レコードの `facts`（`fetched_at` / `route` / `ok` / `http_status` / `text_chars` / `text_sha1` / `image_count` / `video_understood` / `elapsed_ms` / `reason` / `fetcher_version`）を、`fetch_facts/<TARGET>.json` に**1日1ファイル**で書き出す。形式は `{"<rid>": {...facts...,"depth":"...","missing":[...]}, ...}` のJSON。
...
6. `python3 _build_feed.py` を実行してトップ `index.html` を再生成。
7. **自己検証**（手順5で reviews を作った場合）：`index.html` に `<TARGET>` の日付ブロックがあり、テーマ（`.theme`）とまとめ文（`.summary p`）が入っているか確認する。欠けていれば構造ズレなので `reviews/<TARGET>.html` を直し、手順6をやり直す。**加えて、手順4.5でテーマを1つ以上選んだのに「深掘り」節が無い場合は、実際に `WebSearch` を呼び出したかを振り返る**。呼び出していなければ今からでも手順4.5を実行してから `reviews/<TARGET>.html` に反映し、手順6をやり直す。
...
8. **公開（GitHubへ反映）**：対象は `katut-brain/katut-brain.github.io` リポ（Vaultリポではない）。
   ...
   - 押すファイルは **`reviews/<TARGET>.html` と `index.html`、および手順4.2で作った `fetch_facts/<TARGET>.json` の3つだけ**（手順5で作った場合）。これらを **1回の `push_files` 呼び出し**で `main` にコミット（メッセージ `update: <TARGET>`）。
   ...
   - ⚠️ **push_files に渡す前に、送る中身が本物か必ず確認する**（過去に `index.html` の中身が丸ごと `PLACEHOLDER` という仮文字列に置き換わって公開サイトが一時的に壊れた事故が2回発生している）。`push_files` の引数に入れる `index.html`・`reviews/<TARGET>.html` の内容は、**手順5・6で実際に生成・確認したファイルの中身をそのまま使う**（要約・省略・仮置きの文字列で代用しない）。呼び出し直前に、渡す文字列が `<!doctype html>` で始まり `</html>` で終わっているか、`PLACEHOLDER` という語を含んでいないか、文字量が明らかに短すぎない（`index.html`は数万文字規模のはず）かを目視確認してから送信する。
   - **push後、リポジトリ上の内容を読み返して検証する**（自己申告で「push成功」と判断しない）。`index.html` と `reviews/<TARGET>.html` を GitHub から取得し、上と同じ確認（`<!doctype html>`で始まる・`PLACEHOLDER`を含まない・分量が妥当）を行う。異常が見つかったら、正しい内容で即座に再度 `push_files` を実行して直す。
...
## 成功条件
- `date==TARGET` が1件以上 → `reviews/<TARGET>.html` 生成・`index.html` 反映・`main` に push。
- 0件 → reviews は作らず、取り込み差分があれば `index.html` 更新を push、無ければ何もせず正常終了。
