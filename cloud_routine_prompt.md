...
8. **公開（GitHubへ反映）**：対象は `katut-brain/katut-brain.github.io` リポ（Vaultリポではない）。
   ...
   - **フォールバック（スクリプトが失敗したときだけ・順番を守る）**：失敗したパスに限り、従来どおり `push_files` で送ってよい。**ただしフォールバックは「本文を文字列で運ぶ」経路そのものへ戻ることなので、照合できないなら押さない。押してから考えるのでは遅い（押した時点でもう `main` に載っている）。**
     - ① **押す前に、読み返せるかどうかを先に確かめる**（照合の見込みが無いのに押さないため）。クローンにあって**今回のランで一切変更していない**ファイルを1つ選び、照合専用モードを当てる：
       ```bash
       bash push_via_api.sh --verify-only <owner/repo> <未変更の既存ファイル>
       ```
       公開リポなら `requirements.txt`、Vaultリポなら `RULES.md` でよい。**`verify=match` が返らなければ、その夜はそのリポへフォールバックしない**（`gh api` が読めない環境＝押しても化けを検出できない）。`WRITE_PATH: <パス> UNSENT reason=no_verify_channel` を残して、そのファイルは諦める。
     - ② ①が `verify=match` だったときだけ `push_files` で押す。押したら**すぐに同じ照合を当てる**：
       ```bash
       bash push_via_api.sh --verify-only <owner/repo> <押したパス>
       ```
       `verify=match` が出て初めて成功。**目視条件（`<!doctype html>` で始まる・`</html>` で終わる・`PLACEHOLDER` 非含有・分量が妥当）を成功の根拠にしない** —— 2026-09-04 の化け（全角括弧11箇所）はこの4つを全部素通りした。
     - ③ `verify=MISMATCH` が出たら**化けている**。もう一度 `push_files` で押し直し、再度照合する。2回目も一致しなければそのファイルは諦め、`WRITE_PATH: <パス> fallback=push_files verify=GIVEUP` を残す。
     - ④ **フォールバックで押すときのコミットメッセージの末尾に ` [fallback]` を付ける**（例 `update: <TARGET> [fallback]`）。ランのログは流れて消えるが、コミットメッセージは `git log` に残り続けるので、翌朝こちらが `git log --oneline | grep fallback` で拾える。**ファイル本体には警告を書き足さない** —— 押した後に本文を変えると、ローカル原本とリモートが食い違って照合が意味を失う。
     - ⑤ 押さなかったファイルについて: `reviews/<TARGET>.html` はその日の分が公開されない（公開サイトが更新されないので翌朝すぐ分かる）。**ブックマークノートは翌晩の手順7.5が改めて対象にするので失われない**（重複チェックはリポ上の既存ノートを見るため、押していないノートは「まだ無い」と判定される）。
...
8.5. **公開（Vaultリポへ・ブックマークノート）**（手順7.5でノートを新規作成した場合のみ実行）：対象は `katut-brain/obsidian-vault` リポ（手順8の `katut-brain.github.io` とは別リポ）。
   ...
   - フォールバックは**手順8のフォールバック手順①〜⑤をそのまま適用する**。①の「読み返せるか」の確認には Vaultリポの `RULES.md` を使う（クローンにあり、今回のランで触っていない）。目視で frontmatter を眺めるのは照合の代わりにならない。
   - ⚠️ **Vaultリポで照合できないなら押さないこと**（①で `verify=match` が返らない場合）。ここで押さなくても**そのノートは失われない** —— 手順7.5の重複チェックはリポ上の既存ノートを見るので、押していないノートは翌晩「まだ無い」と判定されて改めて対象になる。**押さない側にコストが無いのだから、確認できないまま押す理由も無い。**
