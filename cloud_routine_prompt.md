...
8. **公開（GitHubへ反映）**：対象は `katut-brain/katut-brain.github.io` リポ（Vaultリポではない）。
   ...
   - **フォールバック（スクリプトが失敗したときだけ）**：失敗したパスに限り、従来どおり `push_files` で送ってよい。**ただしフォールバックは、今回直したはずの「本文を文字列で運ぶ」経路そのものに戻ることなので、押した後の照合を必ず同じ強さで行う。**
     - ① 押したら、**すぐに照合専用モードを回す**：
       ```bash
       bash push_via_api.sh --verify-only <owner/repo> <押したパス>
       ```
       これはローカル原本と GitHub 上の実体を SHA-256 で突き合わせるだけで、本文はあなたの文脈を通らない。`verify=match` が出て初めて成功。
     - ② `verify=MISMATCH` が出たら**化けている**。もう一度 `push_files` で押し直し、再度 `--verify-only` を回す。2回目も一致しなければ、そのファイルはその夜は諦め、`WRITE_PATH: <パス> fallback=push_files verify=GIVEUP` を残す。
     - ③ `verify=unreadable`（読み返せない＝`gh api` 自体が通らない環境）のときは、**「押せたが確認できていない」と扱う**。`WRITE_PATH: <パス> fallback=push_files verify=UNCONFIRMED` を残す。成功と書かないこと —— 2026-09-04 の化け（全角括弧11箇所）は `<!doctype html>` で始まるかも `</html>` で終わるかも `PLACEHOLDER` を含まないかも全部素通りした。**目視条件は成功の根拠にならない。**
     - ④ **フォールバックで押すときのコミットメッセージの末尾に ` [fallback]` を付ける**（例 `update: <TARGET> [fallback]`）。`verify` が `UNCONFIRMED` か `GIVEUP` なら ` [fallback-unverified]` にする。ランのログは流れて消えるが、コミットメッセージは `git log` に残り続けるので、翌朝こちらが `git log --oneline | grep fallback` で拾える。**ファイル本体には警告を書き足さない** —— 押した後に本文を変えると、ローカル原本とリモートが食い違って `--verify-only` が意味を失う。
...
8.5. **公開（Vaultリポへ・ブックマークノート）**（手順7.5でノートを新規作成した場合のみ実行）：対象は `katut-brain/obsidian-vault` リポ（手順8の `katut-brain.github.io` とは別リポ）。
   ...
   - フォールバックで `push_files` を使った場合は、**手順8のフォールバック手順①〜④をそのまま適用する**（`--verify-only` での SHA-256 照合／`MISMATCH` なら押し直し／`unreadable` は `UNCONFIRMED` と書き成功と言わない／コミットメッセージ末尾に ` [fallback]`）。目視で frontmatter を眺めるのは照合の代わりにならない。
