...
def _announce_evidence_failure(target, exc):
    """握りつぶすが黙らない。証跡を書けなかったことをログに残す。

    ⚠️ **この行が無いことを「未実行」の根拠にしない**（2026-09-18 Codex 10周目 P1）。
    最初の証跡書き込みに到達する前（import・引数解析の途中や外側 timeout / kill）で
    止まった場合は、証跡もこの行も残らない。証跡が無い理由は「未実行」「到達前に停止」
    「書き込み失敗」の3通りで、判定にはランのログを併せて見ること
    （手順書 `cloud_routine_prompt.md` の手順2.5 の注記と同じ読み方）。
    """
    try:
...
def _write_run_evidence(target, **fields):
    ...
    **どんな失敗でも例外を投げない**（証跡は振り返り本体より優先度が低い。ここで
    ...
    ⚠️ **証跡が無いことを「実行していない」の根拠にしない**（2026-09-18 Codex 10周目 P1）。
    最初の書き込みに到達する前に止まれば、証跡もこの行も残らない。無い理由は
    「未実行」「到達前に停止」「書き込み失敗」の3通りで、判定にはログを併せて見る。
...
def main(argv):
    ...
    # いたら、起動後・完了前に落ちたという意味になる。**逆は言えない**——ファイルが
    # 無いことは「未実行」を意味しない（この行に到達する前に止まれば何も残らない）。
